...
exports.findAll = (req, res) => {
  ...
  Song.findAll({ where: condition })
    .then(data => res.send(data))
    .catch(err => res.status(500).send({ message: err.message || "Some error occurred while retrieving song." }));
};
...
exports.findOne = (req, res) => {
  ...
  Song.findByPk(id, { include: [{ model: Genre }, { model: Artist }, { model: Album } ] })
    .then(data => {
      delete data["dataValues"]["genreId"];
      data["dataValues"]["artists"].forEach(obj => delete obj["dataValues"]["song_artist"]);
      data["dataValues"]["albums"].forEach(obj => delete obj["dataValues"]["song_album"]);
      res.send(data);
    })

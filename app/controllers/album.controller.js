...
exports.create = (req, res) => {
  ...
  const album = {
    name         : req.body.name,
    release_date : req.body.release_date ? req.body.release_date : "1900-01-01",
...
exports.findAll = (req, res) => {
  ...
  Album.findAll({ where: condition })
       .then(data => res.send(data))
       .catch(err => res.status(500).send({ message: err.message || "Some error occurred while retrieving album." }));
};
...
exports.findOne = (req, res) => {
  ...
  Album.findByPk(id, { include: [{ model: Artist }, { model: Song } ] })
       .then(data => {
           data["dataValues"]["artists"].forEach(obj => delete obj["dataValues"]["artist_album"]);
           data["dataValues"]["songs"].forEach(obj => delete obj["dataValues"]["song_album"]);
           res.send(data);
       })
